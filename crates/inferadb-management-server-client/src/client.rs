...
use inferadb_management_types::{ManagementIdentity, Result};
...
impl ServerApiClient {
    ...
    pub fn with_config(
        ...
        let http_client = HttpClient::builder()
            ...
            .build()
            .map_err(|e| {
                inferadb_management_types::Error::External(format!(
        ...
        Fut: std::future::Future<Output = Result<T>>,
    {
        ...
        for attempt in 0..MAX_RETRY_ATTEMPTS {
            let endpoint = match self.get_next_endpoint().await {
                Some(e) => e,
                None => {
                    return Err(inferadb_management_types::Error::External(
        ...
        self.execute_with_failover("create_vault", |endpoint| {
            ...
            async move {
                ...
                if response.status().is_success() {
                    ...
                    Err(inferadb_management_types::Error::External(format!(
                        "Server returned {}: {}",
                        status, body
                    )))
                }
